...
export function buildReminderText(contact: {
  name: string;
  ...
  if (contact.contact_type === "cliente") {
    const purchaseLine =
      contact.last_purchase_value != null
        ...
        : "";
    return `${header}${purchaseLine}\n\nCliente já convertido — mande uma mensagem humanizada para reengajar (ex: "Oi, tudo bem? Passando pra saber como você está.").`;
  }
